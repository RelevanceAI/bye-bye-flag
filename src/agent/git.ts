...
export async function findExistingPR(repoPath: string, flagKey: string): Promise<ExistingPR | null> {
  const branchName = `remove-flag/${flagKey}`;
  try {
...
export async function createPR(
  ...
  const body = generatePRBody(flagKey, keepBranch, agentOutput, branchName, sessionId, workspacePath, relatedRepos);
...
`
  ...
  return `> 🤖 This PR was automatically generated by bye-bye-flag.
...
${!checksPass ? '⚠️ **Some checks failed. Please review before merging.**' : ''}
...
**To resume this Claude session** (from the same machine):
\`\`\`bash
